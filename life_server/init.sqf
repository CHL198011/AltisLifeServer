...
[] spawn {
	while {true} do {
		_now = diag_tickTime;
		_test = "example" callExtension ["post", [ 
			['diag_fps', diag_fps], 
			['diag_fpsMin', diag_fpsMin],
			['missionObjects', count allMissionObjects 'All'],
			['playableUnits', {isPlayer _x} count playableUnits],
			['remote_ai', {(!local _x) && (!isPlayer _x)} count allUnits],
			['local_ai', {local _x} count allUnits],
			['all_ai', count allUnits - count playableUnits]
		]];
		
		"example" callExtension ["post", [ 
			['callExtension', (diag_tickTime - _now)],
			['frame', diag_frameNo],
			['time', time],
			['last_cb', _test]
		]];
...
private _vaultHouse = [[["Altis", "Land_Cargo_House_V1_F"], ["Malden", "Land_Cargo_House_V1_F"], ["tanoa", "Land_Medevac_house_V1_F"]]] call TON_fnc_terrainSort;
private _altisArray = [13065.389,15355.379,6.464]; //NEED TO UPDATE
...
_dome = nearestObject [[13066.763,15354.057,4.422],"Land_Dome_Small_F"];
