...
if ((_veh isKindOf "Car") || (_veh isKindOf "Ship") || (_veh isKindOf "Air")) then {
    if (life_inv_toolkit > 0) then {
        ...
        if (life_interrupted) exitWith {life_interrupted = false; titleText[localize "STR_NOTF_ActionCancel","PLAIN"]; life_action_inUse = false; repairing = 0;};
        if !(isNull objectParent player) exitWith {titleText[localize "STR_NOTF_ActionInVehicle","PLAIN"]; repairing = 0;};
        ...
        _sideRepairArray = LIFE_SETTINGS(getArray,"vehicle_infiniteRepairFalse");
        };
        ...
        titleText[localize "STR_NOTF_RepairedVehicle","PLAIN"];
          ...
          systemChat "You didn't receive any EXP because you are in the SafeZone";
        };
        repairing = 0;
    };
};
