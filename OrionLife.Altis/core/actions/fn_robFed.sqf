...
private["_robber","_shop","_kassa","_ui","_progress","_pgText","_cP","_rip","_Pos","_rndmrk","_mrkstring","_vault","_goldBarAmount"];
...
if(typeOf _vault == "Land_CargoBox_V1_F" && time - (fed_bank getVariable ["reset",time]) < 6000) exitWith {hint "The bank was recently robbed!";}; //Can only rob every 1hr
if(_cops < 10) exitWith {["You cannot rob this shop, 10 or more police need to be on",false,"slow"] call life_fnc_notificationSystem;};
...
5 cutText ["","PLAIN"];
titleText[format["You have recieved %1 gold bars from the robbery, Run!!",[_goldBarAmount] call life_fnc_numberText],"PLAIN"];

//Give goldbars,relic,exp
...
if!(alive _robber) exitWith {};
[0,format["Police News: The bank has just been robbed: the stolen ammount of gold bars was %1 !",[_goldBarAmount] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",west];
[0,format["Police News: The bank has just been robbed: the stolen ammount of gold bars was %1, if you have seen the robbers, contact police!",[_goldBarAmount] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",civilian];
