...
_extDB_notLoaded = "";
diag_log "----------------------------------------------------------------------------------------------------";
diag_log "------------------------------ Starting Orion Altis Life Client Init ------------------------------";
...
[0, "Waiting For Anti-Hack....."] call mav_introcam_fnc_updateCinematicStatus;
sleep 0.8;
[0, "Anti-Hack Successfully Loaded!"] call mav_introcam_fnc_updateCinematicStatus;
sleep 1;
disableUserInput false;
[1, "PRESS SPACE TO START YOUR NEW LIFE"] call mav_introcam_fnc_updateCinematicStatus;
...
/*if (FETCH_CONST(life_adminlevel) > 0) then {
	AdminGetPlayers = {
		...
		_dialog = ((findDisplay 69169) displayCtrl 1500);
		while {!isnull(findDisplay 69169)} do {
			lbClear _dialog;
			{
				if(alive _x) then { 
					_side = "UNKNOWN";
					...
					_dialog lbAdd format["%1 (%2)",_x getVariable["realname", name _x],_side];
				};
	...
	//TODO: Remove profileNamespace after admin menu is closed
	AdminSelected = {
		disableSerialization;
		players = [];
		_dialog = ((findDisplay 69169) displayCtrl 1500);
		...
		_Selected = (players select (_this select 1));
		profileNamespace setVariable ["Admin_Selected",_Selected];
	};
	...
	AdminStartSpectate = {
		if (profileNamespace getVariable "Admin_Selected" == nil) exitWith{};
		_Spectate = profileNamespace getVariable "Admin_Selected";
		//while {true} do {
			_uid = getPlayerUID _Spectate;
			_veh = vehicle _Spectate;
			if (_uid isEqualTo '') exitWith{call AdminStopSpectate;};
			if !(_veh isEqualTo cameraOn)then
			{
				player reveal _Spectate;
				player reveal _veh;
